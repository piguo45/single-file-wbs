...
- This filter bar is **the foundation for the PM's four filter axes (state #91 → delay #92 → assignee #93 → period #94), added incrementally**, sharing the common rules OR-within-axis / AND-across-axes / default = no filtering (a full-column Excel-style filter is deliberately not built — to stay lightweight and honest).
