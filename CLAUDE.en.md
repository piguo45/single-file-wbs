...
11. **Read-only requests** (report / list / what moved / is anything broken) **must not change the JSON.** Say **"no changes"** in your reply. Write the literal phrase **"No changes"** as its own line at the start or end of the reply (not a paraphrase) — people and the check script rely on that exact phrase.
...
- **The same HTML also runs as-is when served over https, e.g. on GitHub Pages** ([demo](https://piguo45.github.io/single-file-wbs/demo.html)). It contains no outbound-request code, so the data you open is processed inside the browser only.
