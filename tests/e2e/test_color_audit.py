...
C = {
 ...
 "トラック枠": lit(r'\.ptrack\{[^}]*border:1px solid (#[0-9a-fA-F]{3,6})', "ptrack-border"),  # 未達トラックの輪郭＝バーとレーン外(行地色)を分ける線
 # 進捗タブは時間タブと別の専用色（#106）。色相・明度は時間タブと同値のまま彩度だけ15%低い＝意味色は同じで、フル幅バーの色の圧だけ下げる
 "進捗実績": V["actual-soft"], "進捗不足": V["short-soft"],
 "進捗完了": lit(r'\.prw\.done \.pbar\.act\{background:(#[0-9a-fA-F]{3,6})', "prw-done"),  # 進捗ビュー：完了バー(グレー)
 "ヘッダ地": V["head-bg"], "列罫線": V["border"],  # 左表ヘッダの地色と列区切り線＝列リサイザの下地
 "列リサイザ": V["accent"],  # 列幅ドラッグのホバー線(#100)。新色は足さず --accent を再利用（.col-resizer:hover::after）
 # 依存タブ(#66)：新色なし。方眼の上に○(--plan-edge)と矢印(--btn-border)が載り、赤が「最長経路＝輪／違反＝塗り」の2役を形で分ける
 "方眼": V["grid"], "依存矢印": V["btn-border"],
 "依存赤": lit(r'\.dsvg \.dnode\.bad \.dcore\{fill:(#[0-9a-fA-F]{3,6})', "deps-red"),
}
...
PAIRS = [
 ...
 # ⑤ 完了行
 ("完了", "2L"), ("完了", "予定枠"),
 # ⑥ 進捗ビュー：同一レーンに 実績(青)→不足分(赤)→未達トラック が左から並び、完了行はグレー1本。時間タブの 実績/over はこのタブに出ないので組まない
 ("進捗実績", "進捗不足"), ("進捗実績", "進捗トラック"), ("進捗実績", "進捗完了"),
 ("進捗不足", "進捗トラック"), ("進捗不足", "進捗完了"), ("進捗トラック", "進捗完了"),
 # レーンの輪郭と、レーンを載せる行の地色（バーが行に沈まないこと）
 ("進捗実績", "トラック枠"), ("進捗不足", "トラック枠"), ("進捗実績", "1L"), ("進捗実績", "proj"),
 # ⑦ 列リサイザのホバー線（#100）× 重なる下地：ヘッダ地色と列区切り線
 ("列リサイザ", "ヘッダ地"), ("列リサイザ", "列罫線"),
 # ⑧ 依存タブ(#66)：方眼の上に○と矢印が重なり、赤(最長経路の輪/違反の塗り)は◇(モーブ)と○の輪の隣に出る
 ("予定枠", "方眼"), ("依存矢印", "方眼"), ("依存赤", "方眼"),
 ("依存赤", "予定枠"), ("依存赤", "依存矢印"), ("依存赤", "MS既定"),
]
