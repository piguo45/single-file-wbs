...
| test_filter.py | フィルタバー（状態3排他／遅延のみ／期間＝今月・軸間AND・親を残す判定・左右の高さ同期・横軸固定・サマリ不変・永続化・全OFFでも落ちない） |
| test_planlog.py | リスケ履歴 `_planLog`（直近トレイル1本・↷N バッジ・クリック吹き出し／↷フォーム＝`plan` 自動更新＋履歴追記／**日付セルの直接編集は「訂正」で履歴を残さない**・変更なしは記録しない・壊れ入力 graceful） |
| test_holiday.py | 祝日（トップレベル `holidays`）：日付ヘッダの赤字＋名称ツールチップ・土日/祝日の列をガント全高で薄ピンク・文字列形と `{date,name}` 形の両対応・不正要素は無視 |
| test_milestone.py | マイルストーンの GUI 編集（`＋MS` 追加／ラベル編集／削除） |
| test_nest.py | 入れ子（子タスク）の編集（`＋子` でリーフ→集計化・総工数と進捗の不変・最後の子を消すと値が親へ戻る・3階層より深い子は作らせない） |
| test_deps.py | 依存タブ（`_deps`／`_pos`）：決定的な自動配置・1マスおきの格子・最長経路の日数と経路・違反の判定・○のホバー/吹き出し・ズーム・`_deps` 無しでは計算を走らせない（C1）・時間タブへの復帰・round-trip と参照の追従 |
| test_plain.py | 簡素表示トグル（ロゴ/版/`Viewer`/タブ名を隠して `WBS` だけにする・localStorage 記憶・再読込で保持） |
